...
def service_status():
    return jsonify({"status": "on" if check_service_status() == STATUS_RUNNING else "off"})

@app.route('/toggle_autostart', methods=['POST'])
def toggle_autostart():
    autostart_file_path = os.path.expanduser("~/.config/autostart/HealthNotifier.desktop")
...
def autostart_status():
    status = "Enabled" if os.path.exists(os.path.expanduser("~/.config/autostart/HealthNotifier.desktop")) else "Disabled"
    return jsonify({"status": status})
